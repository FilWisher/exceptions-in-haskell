...
calls to \mono{throw} are wrapped in a lambda-abstraction and passed to the body.
This means that throw is not directly invoked by the program but via these lambda-abstractions.
...
This implementation means we cannot have variadic numbers of handlers.
Instead, we need to define \mono{try}, \mono{try2}, \dots\ for different numbers of handlers:
\Verbatimcode
  try2 p0 m h1 h2 = withSubCont p0 (\n ->
      pushPrompt p0 (pushSubCont n (m (\x -> throw p0 n h1 x)
                                      (\x -> throw p0 n h2 x))))
\end{Verbatim}
...
This requires exporting the \mono{Prompt} data constructor from the \mono{Prompt} module.
...
\Verbatimcode
try :: ((a -> CC ans b) -> CC ans r) -> (a -> r) -> CC ans r
try m handler =
  withSubCont p0 (\n -> pushPrompt p0 
    (pushSubCont n (m $ \x -> throw n $ return $ handler x)))
...
throw is still wrapped in a lambda abstraction in the definition of try.
This also means that we still require separate definitions for multiple handlers:

\Verbatimcode
try2 m h1 h2 =
  withSubCont p0 (\n -> pushPrompt p0 
    (pushSubCont n (m (\x -> throw n $ return $ h1 x)
                      (\x -> throw n $ return $ h2 x))))
\end{Verbatim}
...
As a simple example:
\Verbatimcode
runTry $ try2 (\thr1 -> \thr2 -> thr1 randomNumber) 
              (\x -> x + 100) -- first handler
              (\x -> x + 1000) -- second handler
\end{Verbatim}

From this example, we can see we still have the same problem with injecting lambdas to correctly bind names.
