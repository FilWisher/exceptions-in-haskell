...
\subsubsection{$\alpha$-reduction}
  ...
  Head reduction is a reduction strategy for the \lam-calculus.
  In head reduction, only the head variables are reduced.
  The head variable $hv$ of a term is defined inductively:
  
  \definition{
  \textsc{Head variable $hv$ for \lam-terms}
  \[
  \begin{array}{rclr}
    hv(`lx.M) &=& hv(M) \\
    hv(xM_1 \dots M_n) &=& x \\
  ...
  By only allowing reduction on head variables, 
  \bta-reduction is restricted to the following contexts:
 
  \definition{
  \textsc{Head reduction for \lam-calculus}
  \[
  \begin{array}{lclrcl}
    M \to N & \Rightarrow &\{&  \\
                           &&& `lx.M & \to_{H} & `lx.N \\
                           &&& M\mult{L} & \to_{H} & N\mult{L} \\
            &             &\}
  \end{array}
  \]
  }
  
  These restricted rules define head reduction for the \lam-calculus. 
  When a term can no longer be \bta-reduced, it is in \bta-normal form.
  Similarly, if a term cannot be head-reduced any further, it is in $`bH$-normal form.
  We can define $`bH$-normal form on \lam-terms with the following BNF grammar:
 
  \definition{
  \textsc{Grammar for \lam-terms in $`bH$-normal form.}
  \[
  \begin{array}{rcl}
    N & ::= & `lx.N \\
      &  |  & xM_1\dots M_2
  ...
  $`bH$-normal form only allows the reduction of the outermost, left-most redex.
  We define $\to_{`bH}^{nf}$ as the reduction of a term to $`bH$-normal form.
  Similarly we use $\to_{`b}^{nf}$ to denote the reduction to $`b$-normal form.
  
  \begin{example}[\lam-terms in \bta- and $`bH$-normal forms]
  \[
  \begin{array}{rcl}
    & \to_{`b}^{nf} & `ly.zy \\
    (`lv.`ly.z(vy))(`lx.x) \\
    & \to_{`bH}^{nf} & `ly.z((`lx.x)y) \\
  \end{array} 
  \]
  \end{example}
...
\section{Continuations}
  ...
  Assume again that we have reduction rules defined for manipulating continuations. 
  By keeping continuations separate, 
  this model would allow us to use parts of the stack selectively for instance taking just top $N$ contexts. 
  The increased granularity of control means we can manipulate not just the entire remaining continuation but sections of it. 
  Continuations of this kind are called \textbf{delimited continuations}.
...
\section{\lmu-Calculus}
  ...
  \lmu\ introduces \lmu-abstractions. 
  The body of a \lmu-abstraction must be a named term. 
  ...
  The idea of head-reduction can be extended to the \lmu-calculus.
  First, we extend the definition of head variables and add the definition of \emph{head names}:
  
  \definition{
  \textsc{Head variables and head names $hn$ for the \lmu-calculus}
  \[
  \begin{array}{lclr}
    hv(`lx.M) &=& hv(M) \\
    hv(xM_1 \dots M_n) &=& x \\
  ...
  Reduction rules can only be applied to the head variable.
  Additionally, $`m$-reduction rules are restricted to terms labelled with the head name:
  
  \definition{
  \textsc{Head reduction for \lmu-calculus}
  \[
  \begin{array}{lclrcl}
    M \to N & \Rightarrow &\{&  \\
                           &&& `lx.M & \to_{H} & `lx.N \\
                           &&& M\mult{L} & \to_{H} & N\mult{L} \\
                           &&& `m`a.[`b]M & \to_{H} & `m`a.[`b]N \\
                           &&& M[[`a]M'L/[`a]M'] & \to_{H} & N[[`a]M'L/[`a]M'] \\
            &             &\}
  \end{array}
  \]
  }
  These restricted rules define head reduction for the \lmu-calculus.
  $`mH$-normal form is defined as terms that cannot be $`mH$-reduced any further.
  Terms in $`mH$-normal form are those producible by the following grammar:
  
  \definition{
  \textsc{Grammar for \lmu-terms in $`b`mH$-normal form.}
  \[
  \begin{array}{rcll}
    N & ::= & `lx.N \\
      &  |  & xM_1\dots M_2 \\
  ...
  The head reduction of \lmu-terms ensures only the left-most and outer-most term is reduced.
  We define $\to_{`mH}^{nf}$ as the reduction to normal form with respect to $\to_{`mH}$.
  
  %\subsection{Types and Isomorphism}
...
\section{Delimited-Continuation Calculus}
  ...
  \begin{figure}[!h]\label{fig:dcc-abstract-machine}
  \relscale{0.9}
  \definition{ 
    ...
    \\
      ...
      \langle withSubCont \ p\ v, D, E, q \rangle &\to &\langle v (D : E\textsmaller[1]{\overset{p}{\uparrow}}, \square, E\textsmaller[1]{\overset{p}{\downarrow}}, q \rangle \\
      \langle pushSubCont E^\prime\ e, D, E, q \rangle &\to &\langle e, \square, E^\prime +{+} (D : E), q \rangle \\
    \\
  ...
  The abstract machine defined in Figure \ref{fig:dcc-abstract-machine} also encodes the reduction strategy.
