...
This grammar describes formul\ae containing any number of occurrences of the letters $t$ or $f$ separated by either an $a$ or an $o$.
...
\section{\lmu-Calculus}
  ...
  Michel Parigot defined the \lmu-calculus as a system with an isomorphism to classical logic.
  ...
  \definition{ 
    ...
    \begin{array}{rrcll}
    i) & (`lx.M) N & \to_{`b} & M[N/x] \\
    ii) & (`m`a.[`b]M) N & \to{`m} & `m`a.[`b](M\{[`g]M^\prime N/[`a]M^\prime\}) & (`b \not= `a)\\
  ...
  % TODO: this is thin, it needs better explanations
